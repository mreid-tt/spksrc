...
service_postinst ()
{
  ...
  ${SYNOPKG_PKGDEST}/bin/initdb -D ${DATABASE_DIR} --encoding=UTF8 --locale=en_US.UTF8
  ...
  ${SYNOPKG_PKGDEST}/bin/psql -p ${PG_PORT} -d postgres -c "ALTER ROLE ${EFF_USER} WITH PASSWORD '${PG_PASSWORD}';"

  # Create new role
  ${SYNOPKG_PKGDEST}/bin/psql -p ${PG_PORT} -d postgres -c "CREATE ROLE ${PG_USERNAME} PASSWORD '${PG_PASSWORD}' SUPERUSER CREATEDB CREATEROLE INHERIT LOGIN REPLICATION BYPASSRLS;"
...
service_preuninst ()
{
  ...
  if [ "$wizard_pg_dump_database" = "true" ]; then
    ...
    do
        # Dump each database
        ${SYNOPKG_PKGDEST}/bin/pg_dump -p ${PG_PORT} -U ${PG_USERNAME} -Fc ${database} >${PG_BACKUP_DUMP_DIR}/${database}.dump
    done
